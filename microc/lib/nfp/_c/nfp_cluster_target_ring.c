...
void cluster_target_ring_put(
    __xwrite void *data,
    unsigned int island_number,
...
)
{
    _INTRINSIC_BEGIN;
    {
        ...
        else
        {
            if (!__is_ct_const(count))
            {
                CT_QPERFINFO_INDIRECT_REF(cls_put_ring);
            }
...
__intrinsic
void cluster_target_ring_init_ptr32(
    unsigned int ring_number,
...
)
{
    _INTRINSIC_BEGIN;
    {
        unsigned int base_address = (unsigned int) (base_ptr) >> 9;

        _CT_RING_BASE_ADDRESS_ALIGN_CHECK((unsigned int)base_ptr);

        cluster_target_ring_init
        (
            0,
            ring_number,
            base_address,
...
__intrinsic
void cluster_target_ring_init_ptr40(
    unsigned int ring_number,
...
)
{
    ...
    {
        unsigned int base_address = (unsigned int)(base_ptr) >> 9;
        unsigned int island_number = (unsigned int)((unsigned long long) base_ptr >> 32) & 0x3f;
...
)
{
...

    {
        ...
        xfer[0] = (((unsigned int)size & 0x07) << 29) | ((status_events & 0x1) << 28) |  ((base_address & 0x1ff) << 9);

        if (sync == sig_done)
        {
            cluster_target_xpb_write((void *)&xfer[0], &xpb_command, 1, sig_done, sig_ptr);
            __wait_for_all(sig_ptr);
        }
        else
        {
            cluster_target_xpb_write((void *)&xfer[0], &xpb_command, 1, ctx_swap, sig_ptr);
        }
        ...
        if (sync == sig_done)
        {
            cluster_target_xpb_write((void *)&xfer[1], &xpb_command, 1, sig_done, sig_ptr);
            __wait_for_all(sig_ptr);
        }
        else
        {
            cluster_target_xpb_write((void *)&xfer[1], &xpb_command, 1, ctx_swap, sig_ptr);
        }
        ...
        /* Do a readback of ring tail to ensure the CSR update is complete before accessing the rings*/
        {
            ...
            if (sync == sig_done)
            {
                cluster_target_xpb_read((void *)&read_xfer, &xpb_command, 1, sig_done, sig_ptr);
            }
            else
            {
                cluster_target_xpb_read((void *)&read_xfer, &xpb_command, 1, ctx_swap, sig_ptr);
            }
        }
    }
